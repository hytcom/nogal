...
class nglFile extends nglBranch implements inglBranch {
	...
	final protected function __declareArguments__() {
		$vArguments							= array();
		$vArguments["content"]				= array('$mValue');
		$vArguments["curl_options"]			= array('$mValue');
		$vArguments["downname"]				= array('(string)$mValue', null);
		$vArguments["filepath"]				= array('(string)$mValue');
		$vArguments["saveaspath"]			= array('(string)$mValue', null);
		$vArguments["hugefile"]				= array('self::call()->istrue($mValue)', false);
		$vArguments["length"]				= array('(int)$mValue', 0);
		$vArguments["mimetype"]				= array('(string)$mValue', null);
		$vArguments["outtype"]				= array('(string)$mValue', null);
		$vArguments["reload"]				= array('self::call()->istrue($mValue)', true);
		return $vArguments;
	}
	...
	public function load() {
		...
		if($bURL) {
			...
			$aBasename = $bIsDir ? array() : explode(".", $vFile["basename"]);
