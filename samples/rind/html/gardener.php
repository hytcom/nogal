...
# INI ==========================================================================
function gardenerError($sMsg) {
	global $GARDENER;
	$GARDENER .= "<h6 class='error'>ERROR, ".$sMsg."</h6>";
	die($GARDENER);
}
...
NOGAL;
if(isset($_SERVER["SHELL"])) { $GARDENER .= " ( using \$_SERVER['PWD'] )"; }
$GARDENER .= <<<NOGAL
				</td></tr>
...
if(!file_exists($CONFIGPATH)) {
	gardenerError("no se pudo hallar el archivo de configuración: ".$CONFIGPATH);
}
require_once($CONFIGPATH);

if(defined("NGL_GARDENER") && !NGL_GARDENER) { die("404 Not Found"); }

if(!is_dir(NGL_PATH_FRAMEWORK)) { gardenerError("NGL_PATH_FRAMEWORK no es una ruta válida: ".NGL_PATH_FRAMEWORK); }
if(!is_dir(NGL_PATH_PROJECT)) { gardenerError("NGL_PATH_PROJECT no es una ruta válida: ".NGL_PATH_PROJECT); }
if(defined("NGL_SOWED")) {
	...
	foreach($CONSTANTS as $key => $value) {
		...
		$value = ($value!==null) ? addcslashes($value, "\r\n\t") : "<i>NULL</i>";
...
$GARDENER .= <<<NOGAL
	<address>
		...
		ATENCION!!! Desactivar el Gardener al finalizar su utilizaci&oacute;n (NGL_GARDENER = false)<br />
		<br />
	</address>
	<table align="center" border="1" class="table table-striped table-bordered">
		{$INFO}
...
NOGAL;
echo NGL_PATH_CONF.NGL_DIR_SLASH."rind.conf";
...
if(isset($_SERVER["SHELL"])) {
	$GARDENER = preg_replace("/<style>(.*?)<\/style>/s", "", $GARDENER);
	$GARDENER = str_replace(
		array("\t", "\n", "<br />", "</th><td>","</td></tr>"),
		array("", "", "\n", "  =  ", "\n"),
		$GARDENER
	);
	echo "\n--------------------------------------------------------------------------------\n\n";
	echo strip_tags($GARDENER);
	echo "\n\n--------------------------------------------------------------------------------\n";
} else {
	die($GARDENER);
}
